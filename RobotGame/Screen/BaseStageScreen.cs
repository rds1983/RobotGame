...
namespace RobotGame
{
	...
	public abstract class BaseStageScreen : GameScreen
	{
		...
		protected RenderTarget2D renderTarget3 = null;
		...
		public void RestoreRenderTarget()
		{
			...
			renderTarget1 = new RenderTarget2D(GraphicsDevice,
											...
											false, format, DepthFormat.Depth24Stencil8);
			renderTarget2 = new RenderTarget2D(GraphicsDevice,
											pp.BackBufferWidth / 4,
											pp.BackBufferHeight / 4,
											false, format, DepthFormat.Depth24Stencil8);
			renderTarget3 = new RenderTarget2D(GraphicsDevice,
											pp.BackBufferWidth,
											pp.BackBufferHeight,
											false, format, DepthFormat.Depth24Stencil8);
		}
		...
		/// </summary>
		internal void OnPostDraw3DScreen(object sender, PostDraw3DEventArgs e)
		{
			...
			DrawPostScreenEffect(e.Scene, postScreenEffect, 0, new Point(renderTarget1.Width, renderTarget1.Height));
			//            DrawPostScreenEffect(renderTarget2, renderTarget1, postScreenEffect, 0);
			...
			GraphicsDevice.Clear(Color.Transparent);
			DrawPostScreenEffect(renderTarget1, gaussianBlurEffect, 0, new Point(renderTarget2.Width, renderTarget2.Height));
			...
			GraphicsDevice.SetRenderTarget(renderTarget1);
			GraphicsDevice.Clear(Color.Transparent);
			DrawPostScreenEffect(renderTarget2, gaussianBlurEffect, 0, new Point(renderTarget1.Width, renderTarget1.Height));
			...
			DrawFinalScreenEffect(renderTarget3, e.Scene, renderTarget1, postScreenEffect, 1);
			...
			for (int index = 0; index < GameLevel.PlayerCountInLevel; index++)
			{
				...
				if (player.IsActiveBooster && !player.IsDelayBooster)
				{
					...
					for (int i = 0; i < boosterEffect.CurrentTechnique.Passes.Count; i++)
					{
						...
						spriteBatch.Draw(renderTarget3,
			...
			int pass, Point size)
		{
			...
			sprite.Begin(SpriteSortMode.Immediate, null);
			//                              SaveStateMode.None);
		...
		/// using a custom shader to apply postprocessing effects.
		/// </summary>
		void DrawFinalScreenEffect(RenderTarget2D drawTo, Texture2D baseTexture, RenderTarget2D renderTarget,
								  Effect effect, int pass)
		{
			Viewport defaultViewport = FrameworkCore.DefaultViewport;
			SpriteBatch sprite = FrameworkCore.RenderContext.SpriteBatch;

			GraphicsDevice.SetRenderTarget(drawTo);
			GraphicsDevice.Clear(Color.Transparent);
